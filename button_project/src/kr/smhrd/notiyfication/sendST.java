package kr.smhrd.notiyfication;
...
public class sendST {
   ...
   public void sendSMS(String phonenumber, String messageContents) {
      ...
      String accessKey = "N7AtOx7oKn3dVGJTRhCy"; // 네이버 클라우드 플랫폼 회원에게 발급되는 개인 인증키 // Access Key :
                                       // https://www.ncloud.com/mypage/manage/info > 인증키 관리 > Access Key
                                       // ID
      String secretKey = "bstAJlOJuT27V83tlahbmsarHimVuTy0tvUol2ur"; // 2차 인증을 위해 서비스마다 할당되는 service secret key //
                                                      // Service Key :
                                                      // https://www.ncloud.com/mypage/manage/info >
                                                      // 인증키 관리 > Access Key ID
      String serviceId = "ncp:sms:kr:287638756846:buttonproject"; // 프로젝트에 할당된 SMS 서비스 ID // service ID :
      ...
      bodyJson.put("from", "01083262142"); // Mandatory, 발신번호, 사전 등록된 발신번호만 사용 가능
      // bodyJson.put("subject",""); // Optional, 기본 메시지 제목, LMS, MMS에서만 사용 가능
      bodyJson.put("content", messageContents); // Mandatory(필수), 기본 메시지 내용, SMS: 최대 80byte, LMS, MMS: 최대
                                             // 2000byte
